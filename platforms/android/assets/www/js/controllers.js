...
.controller('CreateCtrl', function($scope,$http,$location,$ionicHistory,$rootScope,$state,localVariables) {
...
.controller('SinginCtrl', function($scope,$http,$location,$ionicHistory,$ionicPopup,CustomerInfo,$ionicLoading,$rootScope,BuynowFactory,$timeout,customerDetails,localVariables) {
$scope.myGoBack = function() {
...
$ionicLoading.hide();
  ...
  //$rootScope.regvariable = response.data.records[0].customer_id;
     localVariables.setregvariable(response.data.records[0].customer_id);
      ...
      mycall(response.data.records[0].customer_id,CustomerInfo,customerDetails);
  ...
  localVariables.setloginvariable(response.data.records[1]);
    ...
    var registervariable = localVariables.getregvariable(response.data.records[0].customer_id);
     ...
     alert("customerid"+registervariable+"......response"+JSON.stringify(response));
...
.controller('AppCtrl', function($scope,$window,$ionicModal,$ionicHistory, $ionicLoading,$timeout,MainFactory,$ionicPopup,$location,$rootScope,localVariables) {
...
.controller('ShippingAddressCtrl', function($scope,$rootScope,$http,$state,$ionicLoading,$timeout,$ionicPopup,$location,customerDetails,localVariables,jsonFactory) {
...
//End of the controller
 ...
 //this controller
 .controller('paymentctrl', function($scope,$location,$rootScope,$http,$state,$ionicLoading,$timeout,$filter,localVariables,customerDetails) {
$scope.nonce="";
...
type:$scope.type};
console.log(data);
$http.post('http://otcdeal.com/OTCDealWS/PaymentOrder.php11',data).then(
    function successCallback(response) {
...
.controller('MyAccountCtrl', function($scope,$rootScope,$state,$ionicHistory,OrderFactory,$http,$timeout,$ionicLoading,localVariables){
...
.controller('AddressBookCtrl', function($scope,$rootScope,$http,$state,$ionicLoading,$timeout,$ionicPopup,customerDetails,localVariables){
...
$http.post('http://otcdeal.com/OTCDealWS/OTCDealCustomerAddress.php',data).then(
  function successCallback(response) {
...
function mycall(customerid,CustomerInfo,customerDetails){   
